    ...
    fn checkpoint(
        ...
        impl CheckpointCallback for EnsurePage1IsFirstCb<'_> {
            fn frame(&mut self, _frame: &[u8], _page_no: std::num::NonZeroU32, frame_no: std::num::NonZeroU32) -> libsql_sys::wal::Result<()> {
                if frame_no.get() > self.inject_offset {
...
pub(super) fn inject_replication_index<W: Wal>(wal: &mut W) -> libsql_sys::wal::Result<u32> {
    ...
    let last_offset = wal.last_fame_index().map_or(0, |x| x.get());
    page1.header.replication_index += ((last_offset - last_page_1_offset) as u64 + 1).into();
...
mod test {
    ...
    use rusqlite::ffi::{sqlite3_wal_checkpoint_v2, SQLITE_CHECKPOINT_FULL, SQLITE_IOERR_WRITE, SQLITE_CHECKPOINT_RESTART};
    use tempfile::tempdir;
    use tokio::sync::Notify;

    use crate::connection::libsql::{open_conn, open_conn_active_checkpoint};
    ...
    fn inject_replication_index() {
        ...
        let conn = open_conn_active_checkpoint(tmp.path(), wal_manager, None, 1000).unwrap();
    ...
    fn inject_while_holding_write_lock() {
        ...
        let conn2 = open_conn_active_checkpoint(tmp.path(), wal_manager, None, 1000).unwrap();
    ...
    fn multi_connections_edit_page_1() {
        ...
        let conns = (0..3)
            .map(|_| {
                ...
                open_conn_active_checkpoint(tmp.path(), wal_manager, None, 1000000).unwrap()
            })
            .collect_vec();
    ...
    async fn write_between_inject_and_checkpoint() {
        ...
        let conn = open_conn_active_checkpoint(tmp.path(), wal_manager, None, u32::MAX).unwrap();
